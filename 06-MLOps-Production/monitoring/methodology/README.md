...
### 1. 离线评估框架（Offline Evaluation Framework）
    ...
    def compute(self, preds, refs):
        ...
        for n in range(1, self.max_n + 1):
            ...
            for pred, ref in zip(preds, refs):
                ...
                overlap = sum(min(pred_ngrams[k], ref_ngrams[k]) for k in pred_ngrams)
                match += overlap
                total += max(len(pred_ngrams), 1)
            precisions.append(match / total if total > 0 else 0)

        if min(precisions) == 0:
            geo_mean = 0
        else:
            geo_mean = math.exp(sum(math.log(p) for p in precisions) / self.max_n)

        # 简短惩罚
        pred_len = sum(len(p) for p in preds)
        ref_len = sum(len(r) for r in refs)
        bp = 1 if pred_len > ref_len else math.exp(1 - ref_len / max(pred_len, 1))
...
class Perplexity(Metric):
    ...
    def compute(self, token_log_probs):
        # token_log_probs: 每个token的log概率
        n = len(token_log_probs)
        return math.exp(-sum(token_log_probs) / max(n, 1))
...
class Evaluator:
    ...
    def run(self, preds, refs):
        # preds/refs: tokenized list
        results = {}
        for metric in self.metrics:
            results[metric.name] = metric.compute(preds, refs)
        return results
...
preds = [
    ...
    "a quick brown fox".split()
]
refs = [
    "the cat sat on the mat".split(),
    "a fast brown fox".split()
]

evaluator = Evaluator([BLEU(), ROUGE_L()])
results = evaluator.run(preds, refs)
print(results)
```
...
refs = ["The cat sat on the mat.", "A fast brown fox."]
...
def ab_test_mean(a, b, alpha=0.05):
    """均值差异t检验"""
    t, p = stats.ttest_ind(a, b, equal_var=False)
    return {"t": t, "p": p, "significant": p < alpha}

def ab_test_proportion(success_a, n_a, success_b, n_b, alpha=0.05):
    """比例差异z检验"""
    p_a = success_a / n_a
    p_b = success_b / n_b
    ...
    return {"z": z, "p": p_value, "significant": p_value < alpha}
...
def bootstrap_ci(data, num_samples=1000, alpha=0.05):
    ...
    means.sort()
    lower = means[int(alpha/2 * num_samples)]
    upper = means[int((1 - alpha/2) * num_samples)]
    return lower, upper

# 示例
a = [0.12, 0.15, 0.11, 0.14, 0.13]
b = [0.16, 0.17, 0.18, 0.15, 0.16]

print(ab_test_mean(a, b))
print(ab_test_proportion(120, 1000, 150, 1000))
...
print(pass_at_k(200, 30, 1))
```

### 5. 检索指标（Recall@k / MRR / NDCG）
...
def recall_at_k(relevant, retrieved, k):
    ...
    hit = len(set(retrieved_k) & set(relevant))
...
def ndcg(relevant_grades, retrieved, k):
    """计算NDCG@k"""
    def dcg(scores):
        return sum((2**s - 1) / math.log2(i + 2) for i, s in enumerate(scores))

    retrieved_k = retrieved[:k]
    gains = [relevant_grades.get(doc_id, 0) for doc_id in retrieved_k]
    ideal_gains = sorted(relevant_grades.values(), reverse=True)[:k]
    return dcg(gains) / max(dcg(ideal_gains), 1e-9)

# 示例
relevant = ["d1", "d3"]
retrieved = ["d2", "d3", "d1", "d4"]
grades = {"d1": 3, "d3": 2}

print(recall_at_k(relevant, retrieved, k=2))
print(mrr(relevant, retrieved))
print(ndcg(grades, retrieved, k=3))
```

### 6. 校准评估（ECE, Expected Calibration Error）
...
def expected_calibration_error(probs, labels, n_bins=10):
    ...
    for low, high in bins:
        ...
        avg_acc = sum(1 for i in idx if labels[i] == 1) / len(idx)
        ece += abs(avg_conf - avg_acc) * (len(idx) / len(probs))
    return ece

# 示例
probs = [0.9, 0.7, 0.2, 0.4]
labels = [1, 1, 0, 1]
print(expected_calibration_error(probs, labels))
```

### 7. 人评聚合（Pairwise + 置信区间）

```python
import math

def win_rate_with_ci(wins, total, z=1.96):
    """胜率与置信区间"""
    p = wins / total
    se = math.sqrt(p * (1 - p) / total)
    return p, (p - z * se, p + z * se)

def pairwise_result(votes):
    """pairwise投票结果"""
    wins = sum(1 for v in votes if v == "A")
    ties = sum(1 for v in votes if v == "Tie")
    total = len(votes)
    win_rate = wins / total
    return {
        "win_rate": win_rate,
        "tie_rate": ties / total,
        "ci": win_rate_with_ci(wins, total)
    }

votes = ["A", "A", "B", "Tie", "A", "B", "A"]
print(pairwise_result(votes))
```
...
def srm_chi_square(observed, expected):
    """SRM卡方检验"""
...
def cuped(y, x):
    """CUPED方差降低"""
    mean_x = sum(x) / len(x)
    cov = sum((yi - sum(y)/len(y)) * (xi - mean_x) for yi, xi in zip(y, x)) / (len(y) - 1)
    var_x = sum((xi - mean_x) ** 2 for xi in x) / (len(x) - 1)
    theta = cov / var_x if var_x != 0 else 0
    return [yi - theta * (xi - mean_x) for yi, xi in zip(y, x)]

observed = [510, 490]
expected = [500, 500]
print(srm_chi_square(observed, expected))
...
def generate_report(results, meta, output_path="evaluation_report.json"):
    """生成评估报告"""
    report = {
        "meta": meta,
        "results": results,
...
generate_report(results, meta)
```

### 10. RAG评估示例（检索 + 生成）

```python
def rag_evaluate(retrieved_docs, relevant_docs, answers, references):
    """RAG评估：检索Recall + 答案F1"""
    # 检索Recall@k
    recall_scores = [
        recall_at_k(rel, ret, k=5)
    ...
    f1_scores = [f1(a, r) for a, r in zip(answers, references)]

    return {
        "retrieval_recall@5": sum(recall_scores) / len(recall_scores),
...
relevant_docs = [["d2"], ["d4"]]
answers = ["北京是中国的首都", "太阳位于太阳系中心"]
refs = ["中国的首都是北京", "太阳是太阳系中心天体"]

print(rag_evaluate(retrieved_docs, relevant_docs, answers, refs))
```

### 11. LLM-as-a-Judge评分示例

```python
def llm_judge_score(candidate, rubric):
    """伪代码：用LLM进行评分"""
    prompt = f"""
你是评审员，请按照以下评分标准给出1-5分。
标准：{rubric}
回答：{candidate}
"""
    # 伪代码：调用LLM评分
...
rubric = "准确性、简洁性、逻辑性"
candidate = "北京是中国的首都。"
print(llm_judge_score(candidate, rubric))
```

### 12. 错误分析（Error Analysis）

```python
def error_analysis(preds, refs):
    """简单错误分析：统计错误类型"""
    error_types = {"missing": 0, "extra": 0, "mismatch": 0}
    for pred, ref in zip(preds, refs):
        pred_set = set(pred.split())
        ref_set = set(ref.split())
        if pred_set < ref_set:
            error_types["missing"] += 1
        elif pred_set > ref_set:
            error_types["extra"] += 1
        elif pred_set != ref_set:
            error_types["mismatch"] += 1
    return error_types

preds = ["the cat is on mat", "fast brown fox"]
refs = ["the cat sat on the mat", "a fast brown fox"]
print(error_analysis(preds, refs))
```

### 13. 简易毒性评估（Keyword-based）

```python
def toxicity_score(text, toxic_words):
    """简单毒性评分：基于关键词比例"""
    tokens = text.lower().split()
    if not tokens:
        return 0.0
    toxic_count = sum(1 for t in tokens if t in toxic_words)
    return toxic_count / len(tokens)

toxic_words = {"hate", "kill", "stupid"}
text = "I hate this and you are stupid"
print(toxicity_score(text, toxic_words))
```

### 14. 幻觉评估（参考一致性）

```python
def factual_consistency(answer, reference):
    """基于重叠的事实一致性（简化版）"""
    ans_set = set(answer.split())
    ref_set = set(reference.split())
    if not ans_set:
        return 0.0
    precision = len(ans_set & ref_set) / len(ans_set)
    recall = len(ans_set & ref_set) / len(ref_set) if ref_set else 0
    return 2 * precision * recall / (precision + recall) if (precision + recall) else 0

answer = "北京 是 中国 的 首都"
reference = "中国 的 首都 是 北京"
print(factual_consistency(answer, reference))
```
...
text = "请联系我：test@example.com 或 123-456-7890"
...
class EvalConfig:
    def __init__(self, model_name, dataset_path, prompt_version):
...
def evaluate_dataset(dataset, metrics):
    """运行评估"""
    preds, refs = [], []
    for item in dataset:
        prompt = item["prompt"]
        pred = mock_generate(prompt)
        preds.append(pred)
...
config = EvalConfig("llm-v3", "eval_set.json", "prompt-v2")
dataset = [
    {"prompt": "什么是机器学习？", "reference": "机器学习是让模型从数据中学习规律。"}
]

results = evaluate_dataset(dataset, [BLEU(), ROUGE_L()])
print(results)
```
...
```python
def prompt_sensitivity_test(prompts, question):
    """测试不同Prompt对结果的影响"""
    outputs = []
    for p in prompts:
        full_prompt = f"{p}\n问题：{question}\n答案："
        outputs.append(mock_generate(full_prompt))
    return outputs
...
outputs = prompt_sensitivity_test(prompts, "什么是强化学习？")
print(outputs)
```
...
def plot_metric_comparison(labels, values):
    """绘制指标对比图"""
...
values = [0.31, 0.35]
plot_metric_comparison(labels, values)
```
...
[
  {
    "id": "qa-001",
    "prompt": "解释什么是过拟合？",
    "reference": "过拟合是模型在训练集表现好但泛化差的现象。",
    "metadata": {
      "domain": "ML",
      "difficulty": "medium"
    }
