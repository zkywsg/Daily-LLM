...
2. VAE 如何用"学分布"代替"学向量"实现生成？重参数化技巧解决了什么问题？
3. GAN 的博弈训练框架为什么能工作，训练不稳定的原因是什么？
4. 编码器-解码器架构如何在分割和生成中分别发挥作用？
5. DCGAN 的工程实践解决了哪些稳定性问题？Progressive GAN 的渐进式策略为什么有效？
...
### 2.3 VAE：编码器-解码器的另一种生成范式

U-Net 展示了编码器-解码器的"分割"能力——把图像压缩再展开，得到逐像素的语义标签。同样的编码器-解码器结构能不能反过来做"生成"？自编码器（Autoencoder）给出了朴素的答案：编码器把图像压缩成潜向量 $\mathbf{z}$，解码器从 $\mathbf{z}$ 重建原图。但普通自编码器有一个致命缺陷：**潜空间是不规则的**——编码器可能把不同图片映射到潜空间的任意位置，你无法保证从潜空间随机采一个点，解码器就能生成有意义的图像。

Kingma & Welling (2013) 提出的变分自编码器（VAE）用了一个优雅的思路来解决这个问题：**不学一个确定的潜向量，而是学一个分布**。

**编码器不输出 $\mathbf{z}$，而是输出均值 $\boldsymbol{\mu}$ 和方差 $\boldsymbol{\sigma}^2$**——它描述的是"潜变量大概在哪个区域"，而不是一个精确的点。然后从 $q(\mathbf{z}|\mathbf{x}) = \mathcal{N}(\boldsymbol{\mu}, \text{diag}(\boldsymbol{\sigma}^2))$ 中采样得到 $\mathbf{z}$，再交给解码器重建。

但采样操作不可微，梯度无法回传。**重参数化技巧（Reparameterization Trick）**解决了这个问题：

$$\mathbf{z} = \boldsymbol{\mu} + \boldsymbol{\sigma} \odot \boldsymbol{\epsilon}, \quad \boldsymbol{\epsilon} \sim \mathcal{N}(0, \mathbf{I})$$

随机性被转移到外部变量 $\boldsymbol{\epsilon}$ 上，$\boldsymbol{\mu}$ 和 $\boldsymbol{\sigma}$ 作为确定性运算参与梯度计算，整个流程变得可微。

**VAE 的损失函数由两部分组成：**

$$\mathcal{L}_{VAE} = \underbrace{\mathcal{L}_{recon}}_{\text{重建损失}} + \underbrace{D_{KL}(q(\mathbf{z}|\mathbf{x}) \| p(\mathbf{z}))}_{\text{KL 散度}}$$

- **重建损失**：衡量解码器输出与原始输入的差异（MSE 或 BCE），保证模型能"记住"输入长什么样。
- **KL 散度**：约束编码器输出的分布 $q(\mathbf{z}|\mathbf{x})$ 接近标准正态分布 $p(\mathbf{z}) = \mathcal{N}(0, \mathbf{I})$。这保证了潜空间是连续、平滑的——相邻的点解码后会产生相似的输出。

```mermaid
graph LR
    X["输入 x"]:::input
    ENC["编码器<br/>输出 μ, σ²"]:::compute
    EPS["ε ~ N(0, I)"]:::input
    SAMPLE["z = μ + σ·ε<br/>(重参数化)"]:::compute
    DEC["解码器<br/>重建 x̂"]:::compute
    XHAT["输出 x̂"]:::output
    KL["KL 散度<br/>q(z|x) ≈ N(0,I)"]:::compute
...
**VAE vs GAN：两条生成路线的对比**

| 维度 | VAE | GAN |
|------|-----|-----|
| 训练方式 | 单网络 + 正则化（KL 散度） | 两个网络对抗 |
| 训练稳定性 | 高（损失函数有解析形式） | 低（对抗训练难以平衡） |
| 生成质量 | 偏模糊（重建损失倾向于输出均值） | 更锐利（对抗损失不取平均） |
| 潜空间结构 | 连续平滑，支持插值和采样 | 不规则，采样不可靠 |
| 理论基础 | 变分推断（严格的概率框架） | 极小极大博弈（无显式似然） |
| 评估指标 | 重建误差 + ELBO | FID / IS |

VAE 的输出之所以偏模糊，根源在于重建损失（MSE/BCE）：当模型不确定某个像素应该是哪个值时，最安全的策略是输出所有可能值的**均值**——这在视觉上就表现为模糊。GAN 的判别器不接受"均值式的妥协"，它要求生成样本在细节上也必须逼真，所以 GAN 的输出更锐利。

2020 年之后的扩散模型（DDPM）某种程度上统合了 VAE 和 GAN 的优点：像 VAE 一样有扎实的概率框架，像 GAN 一样能生成高质量样本。

> 你要记住：VAE 的核心创新是"学分布而非学向量"。重参数化技巧让采样可微，KL 散度让潜空间变得规则。VAE 训练稳定但输出模糊，GAN 输出锐利但训练不稳定——这个矛盾后来被扩散模型解决。
...
| 年份 | 论文 | 核心贡献 |
|------|------|----------|
| 2013 | Kingma & Welling *Auto-Encoding Variational Bayes* | VAE，变分推断 + 重参数化，生成模型的概率框架 |
...
| VAE | 学潜空间分布而非确定向量，实现可采样的生成模型 | 重参数化 + KL 散度 |
...
| 维度 | 语义分割（FCN/U-Net） | 变分生成（VAE） | 对抗生成（GAN/DCGAN） |
|------|----------------------|----------------|----------------------|
| 任务类型 | 判别式（判别每个像素的类别） | 生成式（从潜变量采样生成新样本） | 生成式（从噪声生成新样本） |
| 输入 | 图像 | 图像（训练）/ 噪声（采样） | 随机噪声向量 |
| 输出 | H×W×C 的分割图 | H×W×3 的重建/生成图像 | H×W×3 的生成图像 |
| 损失函数 | 逐像素交叉熵 | 重建损失 + KL 散度（ELBO） | 对抗损失（min-max 博弈） |
| 训练稳定性 | 高（标准监督学习） | 高（有解析损失函数） | 低（对抗训练） |
| 生成质量 | — | 偏模糊 | 锐利 |
| 潜空间 | — | 连续平滑，可插值 | 不规则 |
| 评估指标 | mIoU、Pixel Accuracy | 重建误差 + ELBO | FID、IS、人眼评估 |
| 数据需求 | 图像 + 像素级标注（昂贵） | 仅图像（无需标注） | 仅图像（无需标注） |
| 共享架构 | 完整编码器-解码器 | 完整编码器-解码器 | 仅解码器（生成器） |
| 跳跃连接 | 关键（U-Net 的核心） | 无 | 无（或后期工作如 StyleGAN 的自适应） |

这个对比揭示了生成模型的两条路线：VAE 用概率框架保证潜空间规则、训练稳定，但代价是输出模糊；GAN 用对抗训练追求锐利的输出，但代价是训练不稳定和模式坍塌。分割任务不需要做这个权衡——它有清晰的监督信号（逐像素标签），所以训练最稳定。
