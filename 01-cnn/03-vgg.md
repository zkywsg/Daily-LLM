...
**为什么只用 3×3** 是这篇论文的核心论点。单层 3×3 视野有限，但关键在于**堆叠**。两个 3×3 卷积叠起来，第二层的每个神经元能看到的输入区域是 5×5：
...
也就是说，用 3 个 3×3 替换 1 个 7×7，**参数量降低约 45%，同时网络深度增加了 2 层、ReLU 非线性增加了 2 次**——在参数、深度、非线性三个维度上同时改善。这是 VGG 论文的核心结论：小 kernel 堆叠在三个维度上均优于大 kernel。

整张网络的**结构高度规整**：所有卷积 3×3/s=1/p=1，所有池化 2×2/s=2，每段 block 内通道不变、跨 block 翻倍。这种"统一性（uniformity）"在 VGG 之前未被作为明确的设计目标——AlexNet 那种"第一层 11×11、第二层 5×5、后面 3×3"的混合设计在后续工作中被逐步替代，**统一的小 kernel + 模块化 block 成为后续视觉网络的常见起点**（Inception 的 stem 之外都是 3×3 + 1×1；ResNet 的 basic block 是两层 3×3；这种"3×3 砖头"思维直接源自 VGG）。

但 VGG 也存在显著的参数开销问题。VGG-16 有大约 **138M 参数**，其中绝大多数集中在最后三个全连接层——尤其 fc6 把 $7 \times 7 \times 512 = 25088$ 维特征压到 4096 维，单这一层就占约 **102M 参数**，约占全网 74%。卷积层合计仅 14.7M。也就是说，**VGG 名义上是"深 CNN"，但参数主要分布在 FC 层**。这一观察直接影响了 Inception 用全局平均池化替代 fc6 的设计（GoogLeNet 仅 6.8M 参数），也是 1×1 卷积降维方案的引入背景。
