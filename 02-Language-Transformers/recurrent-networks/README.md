...
把它沿时间展开后，可以把网络看成一条不断传递隐藏状态的链；但每个时间步复用的仍是同一组参数 $W_h$、$W_x$ 和 $b$。这就是 RNN 能处理变长输入、同时把历史信息持续带到后续步骤的原因。

### 2.2 为什么难训：BPTT、梯度消失与梯度爆炸

反向传播会沿时间链条逐步展开，长链意味着梯度会不断连乘：
- 小于 1 时快速衰减
- 大于 1 时快速放大
...
LSTM 在隐藏状态之外再维护一条细胞状态通道，核心更新如下：
...
其中，遗忘门决定旧记忆保留多少，输入门决定当前候选信息写入多少，输出门决定细胞状态有多少会暴露为当前隐藏状态。门控的本质，是把“保留什么、写入什么、读出什么”拆开控制。

### 2.4 GRU：更轻量的门控

GRU 把状态更新进一步简化，关键公式如下：
...
两者的差异可以从结构复杂度、训练代价和适用场景三个方面快速比较：
...
Seq2Seq 用编码器-解码器结构把“读入一个序列”扩展成“生成另一个序列”，关键过程是：
- 编码器把输入序列压成上下文向量
- 解码器根据这个向量逐步生成输出
...
可以按下面四个渐进步骤理解工程实现，每一步都对应一个常见问题：

```python
# Step 1 最小循环编码：先用最小 LSTM 跑通序列编码
import torch
...
```python
# Step 2 变长序列与 padding 安全：不能直接取 out[:, -1, :]
import torch
import torch.nn as nn
...
```python
# Step 3 工程完善：长序列训练容易梯度爆炸，需要先做裁剪再更新
import torch
import torch.nn as nn
