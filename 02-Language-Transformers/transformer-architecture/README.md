...
Transformer 家族的选择主要看输入输出关系：

| 范式 | 结构 | 适合的任务 | 典型模型 |
|------|------|------------|----------|
| Encoder-only | 只保留编码器，双向看上下文 | 分类、检索、序列标注 | BERT、RoBERTa |
| Decoder-only | 只保留解码器，自回归预测下一个 token | 生成、对话、代码补全 | GPT、LLaMA |
| Encoder-Decoder | 编码输入，再逐步解码输出 | 翻译、摘要、改写 | T5、BART |

简单选法：理解类任务优先 Encoder-only，生成类任务优先 Decoder-only，输入到输出转换类任务优先 Encoder-Decoder。
...
**Step 4：最小模型接线**（解决输入输出张量接通）

```python
class Transformer(nn.Module):
    # 解决输入到输出头的张量接线
    # 这里只保留最小骨架，不声称完整的 encoder-decoder 堆栈
