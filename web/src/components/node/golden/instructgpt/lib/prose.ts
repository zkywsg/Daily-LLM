export const INSTRUCTGPT_SOURCE_PATH = "12-rlhf-alignment/02-instructgpt.md";
...
export interface ProseSections {
  ...
  mechanism3: string;
  synergy: string;
  promptDiversity: string;
  /** ## "对齐胜过规模"的实证 */
  alignmentBeatsScale: string;
  /** ## Alignment Tax */
  alignmentTax: string;
  /** ## ChatGPT 是 InstructGPT 的部署版 */
  chatgptDeployment: string;
  /** ## 数据规模(标注员的工作) */
  dataScale: string;
...
const H3_KEYS: Array<{ test: RegExp; key: keyof ProseSections }> = [
  ...
  { test: /^Prompt 多样性/, key: "promptDiversity" },
];

const H2_KEYS: Array<{ test: RegExp; key: keyof ProseSections | "_coreInsight" }> = [
  { test: /^前作进展/, key: "previousWork" },
  { test: /^核心思想/, key: "_coreInsight" },
  { test: /^["“]对齐胜过规模["”]/, key: "alignmentBeatsScale" },
  { test: /^Alignment Tax/i, key: "alignmentTax" },
  { test: /^ChatGPT/, key: "chatgptDeployment" },
  { test: /^数据规模/, key: "dataScale" },
