...
// 不加 f·P aux loss → 极不均匀;加了 → 接近均匀。

const TARGET = 1.0;

export function LoadBalancingDiagram({ balanced }: Props) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", fontFamily: "system-ui" }} role="img" aria-label={`Expert load distribution ${balanced ? "balanced" : "imbalanced"}`}>
      <text x={W / 2} y={20} textAnchor="middle" fontSize={12} fontWeight={600} fill="var(--ink-primary)">
        Expert 负载分布 {balanced ? "(加 f·P aux loss · 均匀)" : "(无 aux loss · 塌缩)"}
      </text>
      <text x={W / 2} y={36} textAnchor="middle" fontSize={11} fontStyle="italic" fill="var(--ink-muted)">
        每 expert 分到的 token 数(相对单位,1.0 = 均匀理想值)
      </text>
      ...
      {loads.map((l, i) => {
        ...
        const tooHot = l > 2;
        const tooCold = l < 0.5;
        const color = tooHot ? "#dc2626" : tooCold ? "#9ca3af" : "#f59e0b";
        return (
          <g key={i}>
            <rect x={x} y={H - PAD.bottom - h} width={barW} height={h} rx={3} fill={color} opacity={0.85} />
      ...
      <text x={W / 2} y={H - 12} textAnchor="middle" fontSize={10} fontStyle="italic" fill={balanced ? "#10b981" : "#dc2626"}>
        {balanced
          ? "✓ 所有 expert 都在工作 · top-1 也能均衡"
          : "✗ E2/E3/E5/E7 几乎闲置 · top-1 without aux loss 极易塌缩到几个热门 expert"}
      </text>
    </svg>
  );
}
